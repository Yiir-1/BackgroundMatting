...
def train_worker(rank, addr, port):
    ...
    model_distributed = nn.parallel.DistributedDataParallel(model, device_ids=[rank],find_unused_parameters=True)
    ...
    optimizer = Adam([
        ...
        {'params': model.aspp.parameters(), 'lr': 1e-5},
        {'params': model.decoder.parameters(), 'lr': 1e-5},
        {'params': model.refiner.parameters(), 'lr': 1e-5},
    ])
    scaler = GradScaler()
