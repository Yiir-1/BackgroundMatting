...
def train_worker():
    ...
    # Run loop
    for epoch in range(args.epoch_start, args.epoch_end):#这里的大小是2048*2048
        for i, ((true_pha, true_fgr), true_bgr) in enumerate(tqdm(dataloader_train)):
            ...
            true_pha, true_fgr, true_bgr = random_crop(true_pha, true_fgr, true_bgr)#这里随即裁剪（因为没有设置种子，所以每次都不太一样，现在是2*c*1260*1088
            ...
            true_src = true_fgr * true_pha + true_src * (1 - true_pha)#得到的是一张真正的图片
...
def random_crop(*imgs):
    ...
    for img in imgs:
        ...
        img = kornia.resize(img, (H_tgt, W_tgt))
        results.append(img)
    return results
